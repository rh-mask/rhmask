...
Solidity for VeilStreet on Robinhood Chain (4663). Foundry project; nothing here is deployed until it has been audited.
...
| `VeilVault` | Stake `$VEIL`, receive a pro-rata stream of stock tokens. Rewards are pushed as ERC-20 stock tokens; the vault never mints anything. | none (no owner, no pause, no upgrade) |
| `RevenueRouter` | Receives ETH / ERC-20 revenue (routing fees, token trading fees, relay fees), swaps to the basket through a fixed DEX route, and forwards to `VeilVault`. | basket weights set by staker vote, timelocked |
